...
def eval_rbbox_map(det_results,
    ...
    print_map_summary(
     ...
     # Save PR curve if save_path is provided
    if save_path:
        save_pr_curve(recalls, precisions, ap, save_path)
    return mean_ap, eval_results

def save_pr_curve(recalls, precisions, ap, save_path):
    """Save PR curve for a single class."""
    plt.figure(figsize=(8, 6))
    plt.plot(recalls, precisions, label=f'AP={ap:.4f}', color='b')
    plt.xlabel('Recall')
